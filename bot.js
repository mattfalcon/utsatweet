...
client.get('search/tweets', {q: "UTSA", count: 30}, function(error, tweets, response) {
  ...
  client.post('statuses/update', {status: selected_tweet.text + "  Tweet Frm:" + selected_tweet.location + ' #UTSA '}, function(error, tweet, response) {
    if (!error) {
      console.log(tweet);
    }
    }); 
